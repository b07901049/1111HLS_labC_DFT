...
aesl_fh.read(AUTOTB_TVOUT_PC_real_op, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<4>((char*)__xlx_apatb_param_real_op, 1024);
}
#else
{
      static ifstream rtl_tv_out_file;
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_real_op);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > real_op_pc_buffer(1024);
          int i = 0;
          ...
          while (AESL_token != "[[/transaction]]"){
            ...
            if (AESL_token != "") {
              real_op_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (has_unknown_value) {
            cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'x' or 'X' on port " 
                 << "real_op" << ", possible cause: There are uninitialized variables in the C design."
                 << endl; 
          }
  
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 1024; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_real_op)[j*4+0] = real_op_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_real_op)[j*4+1] = real_op_pc_buffer[i].range(15, 8).to_int64();
((char*)__xlx_apatb_param_real_op)[j*4+2] = real_op_pc_buffer[i].range(23, 16).to_int64();
((char*)__xlx_apatb_param_real_op)[j*4+3] = real_op_pc_buffer[i].range(31, 24).to_int64();
}}}
        } // end transaction
      } // end file is good
...
{
transaction<32> tr(1024);
aesl_fh.read(AUTOTB_TVOUT_PC_imag_op, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<4>((char*)__xlx_apatb_param_imag_op, 1024);
}
#else
{
      static ifstream rtl_tv_out_file;
      if (!rtl_tv_out_file.is_open()) {
        rtl_tv_out_file.open(AUTOTB_TVOUT_PC_imag_op);
        if (rtl_tv_out_file.good()) {
          rtl_tv_out_file >> AESL_token;
          if (AESL_token != "[[[runtime]]]")
            exit(1);
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<32> > imag_op_pc_buffer(1024);
          int i = 0;
          bool has_unknown_value = false;
          rtl_tv_out_file >> AESL_token; //data
          while (AESL_token != "[[/transaction]]"){
            ...
            // push token into output port buffer
            if (AESL_token != "") {
              imag_op_pc_buffer[i] = AESL_token.c_str();;
              i++;
            }
          ...
          if (has_unknown_value) {
            cerr << "WARNING: [SIM 212-201] RTL produces unknown value 'x' or 'X' on port " 
                 << "imag_op" << ", possible cause: There are uninitialized variables in the C design."
                 << endl; 
          }
  
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 1024; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_imag_op)[j*4+0] = imag_op_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_imag_op)[j*4+1] = imag_op_pc_buffer[i].range(15, 8).to_int64();
((char*)__xlx_apatb_param_imag_op)[j*4+2] = imag_op_pc_buffer[i].range(23, 16).to_int64();
((char*)__xlx_apatb_param_imag_op)[j*4+3] = imag_op_pc_buffer[i].range(31, 24).to_int64();
}}}
        } // end transaction
      } // end file is good
    } // end post check logic bolck
...
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_real_op, 'b');
transaction<32> tr(1024);
  __xlx_offset_byte_param_real_op = 0*4;
  if (__xlx_apatb_param_real_op) {
tr.import<4>((char*)__xlx_apatb_param_real_op, 1024, 0);
  }
aesl_fh.write(AUTOTB_TVIN_real_op, tr.p, tr.tbytes);
}

  tcl_file.set_num(1024, &tcl_file.real_op_depth);
#else
// print real_op Transactions
{
aesl_fh.write(AUTOTB_TVIN_real_op, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_real_op = 0*4;
if (__xlx_apatb_param_real_op) {
for (size_t i = 0; i < 1024; ++i) {
unsigned char *pos = (unsigned char*)__xlx_apatb_param_real_op + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVIN_real_op, s);
}
}
}

  tcl_file.set_num(1024, &tcl_file.real_op_depth);
aesl_fh.write(AUTOTB_TVIN_real_op, end_str());
}
...
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVIN_imag_op, 'b');
transaction<32> tr(1024);
  __xlx_offset_byte_param_imag_op = 0*4;
  if (__xlx_apatb_param_imag_op) {
tr.import<4>((char*)__xlx_apatb_param_imag_op, 1024, 0);
  }
aesl_fh.write(AUTOTB_TVIN_imag_op, tr.p, tr.tbytes);
}

  tcl_file.set_num(1024, &tcl_file.imag_op_depth);
#else
// print imag_op Transactions
{
aesl_fh.write(AUTOTB_TVIN_imag_op, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_imag_op = 0*4;
if (__xlx_apatb_param_imag_op) {
for (size_t i = 0; i < 1024; ++i) {
unsigned char *pos = (unsigned char*)__xlx_apatb_param_imag_op + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVIN_imag_op, s);
}
}
}

  tcl_file.set_num(1024, &tcl_file.imag_op_depth);
aesl_fh.write(AUTOTB_TVIN_imag_op, end_str());
}
...
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVOUT_real_op, 'b');
transaction<32> tr(1024);
  __xlx_offset_byte_param_real_op = 0*4;
  if (__xlx_apatb_param_real_op) {
tr.import<4>((char*)__xlx_apatb_param_real_op, 1024, 0);
  }
aesl_fh.write(AUTOTB_TVOUT_real_op, tr.p, tr.tbytes);
}
...
// print real_op Transactions
{
aesl_fh.write(AUTOTB_TVOUT_real_op, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_real_op = 0*4;
if (__xlx_apatb_param_real_op) {
for (size_t i = 0; i < 1024; ++i) {
unsigned char *pos = (unsigned char*)__xlx_apatb_param_real_op + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVOUT_real_op, s);
}
}
}

  tcl_file.set_num(1024, &tcl_file.real_op_depth);
aesl_fh.write(AUTOTB_TVOUT_real_op, end_str());
}

#endif
#ifdef USE_BINARY_TV_FILE
{
aesl_fh.touch(AUTOTB_TVOUT_imag_op, 'b');
transaction<32> tr(1024);
  __xlx_offset_byte_param_imag_op = 0*4;
  if (__xlx_apatb_param_imag_op) {
tr.import<4>((char*)__xlx_apatb_param_imag_op, 1024, 0);
  }
aesl_fh.write(AUTOTB_TVOUT_imag_op, tr.p, tr.tbytes);
}
...
// print imag_op Transactions
{
aesl_fh.write(AUTOTB_TVOUT_imag_op, begin_str(AESL_transaction));
{
  __xlx_offset_byte_param_imag_op = 0*4;
if (__xlx_apatb_param_imag_op) {
for (size_t i = 0; i < 1024; ++i) {
unsigned char *pos = (unsigned char*)__xlx_apatb_param_imag_op + i * 4;
std::string s = formatData(pos, 32);
aesl_fh.write(AUTOTB_TVOUT_imag_op, s);
}
}
}

  tcl_file.set_num(1024, &tcl_file.imag_op_depth);
aesl_fh.write(AUTOTB_TVOUT_imag_op, end_str());
}
